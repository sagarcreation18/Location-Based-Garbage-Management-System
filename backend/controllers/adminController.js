...
exports.deleteRoute=async(req,res)=>{const connection=await pool.getConnection();try{await connection.beginTransaction();const[bins]=await connection.execute("SELECT bin_id FROM route_bins WHERE route_id=?",[req.params.id]);const[r]=await connection.execute("DELETE FROM routes WHERE id=?",[req.params.id]);if(!r.affectedRows){await connection.rollback();return res.status(404).json({success:false,message:"Route not found"});}if(bins.length){const ids=bins.map(row=>row.bin_id);const placeholders=ids.map(()=>"?").join(",");await connection.execute(`UPDATE garbage_bins SET assigned_driver_id=NULL,route_order=0 WHERE id IN (${placeholders})`,ids);}await connection.commit();res.json({success:true,message:"Route deleted and its bins were released"});}catch(e){await connection.rollback();fail(res,e,"Unable to delete route");}finally{connection.release();}};
