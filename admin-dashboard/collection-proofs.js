(() => {
  ...
  // Uploads are served from the backend origin, not from the /api endpoint.
  // This keeps proof photos working on both localhost and the Railway domain.
  const proofImageUrl = imagePath => String(window.EcoSmartApiBase || `${location.origin}/api`).replace(/\/api\/?$/, "") + String(imagePath || "");
